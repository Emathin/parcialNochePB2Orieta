...
public class Transferencia extends Monetaria implements Rechazable {

	private Boolean bloqueada;
	private Boolean sospechoso;
	private Boolean fueFraude;

	public Transferencia(Cliente queTransfiereDinero, Cliente queRecibeDinero, EntidadBancaria entidad,
			Double montoATransferir) throws FraudeException {
	...
	public void monitorear(Cliente queTransfiereDinero, Cliente queRecibeDinero, Dispositivo deQuienTransfiere,
			Dispositivo deQuienRecibe, EntidadBancaria entidad, Double montoATransferir)
			throws FraudeException {

		Integer score = 0;

		if (entidad.getConjuntoDenunciables().contains(queTransfiereDinero)) {
			score += 80;
			// 1. Si el CUIT del cliente de la transacción formó parte de un fraude se suma
			// 80 puntos al SCORE.
		}
		
		if (entidad.getConjuntoDenunciables().contains(queRecibeDinero)) {
			score += 80;
		}

		if (entidad.getConjuntoDenunciables().contains(deQuienTransfiere)) {
			score += 80;
			// 2. Si la dirección IP o IMEI del dispositivo estuvo involucrado en un evento
			// de FRAUDE, se suma 80 puntos al SCORE.
		}
		
		if (entidad.getConjuntoDenunciables().contains(deQuienRecibe)) {
			score += 80;
		}
		...
		if (queTransfiereDinero.getFondos() == montoATransferir) {
			score += 40;
		...
		if (!queTransfiereDinero.getConjuntoDispositivos().contains(deQuienTransfiere)) {
			score += 20;
			// 5. Si el cliente está operando desde un dispositivo que nunca utilizó
			// previamente, suma 20 puntos al SCORE.
		}

		if (!queRecibeDinero.getConjuntoDispositivos().contains(deQuienRecibe)) {
			score += 20;
		}
